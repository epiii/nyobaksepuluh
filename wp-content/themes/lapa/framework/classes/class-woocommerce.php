...
if(!class_exists('Lapa_WooCommerce')) {

    class Lapa_WooCommerce{
        ...
        public function __construct(){
            ...
            add_filter('lapa/get_site_layout', array( $this, 'set_site_layout') );

            add_filter('lapa/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
            ...
            add_filter('woocommerce_loop_add_to_cart_link', array( $this, 'woocommerce_loop_add_to_cart_link'), 9, 3);
            ...
            add_action('lapa/action/shop_loop_item_action_top', array( $this, 'add_wishlist_btn' ), 1 );
            add_action('lapa/action/shop_loop_item_action_top', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('lapa/action/shop_loop_item_action_top', array( $this, 'add_compare_btn' ), 10 );
            add_action('lapa/action/shop_loop_item_action_top', array( $this, 'add_cart_btn' ), 15 );
            add_action('lapa/action/shop_loop_item_action_top', array( $this, 'add_quick_view_btn' ), 20 );

            add_action('lapa/action/shop_loop_item_action_top', function(){ echo '</div>'; }, 25 );


            add_action('lapa/action/shop_loop_item_action_top', array( $this, 'add_count_up_timer_in_product_listing' ), 40 );

            add_action('lapa/action/shop_loop_item_action', function(){ echo '<div class="wrap-addto">'; }, 5 );
            add_action('lapa/action/shop_loop_item_action', array( $this, 'add_compare_btn' ), 10 );
            add_action('lapa/action/shop_loop_item_action', array( $this, 'add_cart_btn' ), 15 );
            add_action('lapa/action/shop_loop_item_action', array( $this, 'add_wishlist_btn' ), 20 );
            add_action('lapa/action/shop_loop_item_action', array( $this, 'add_quick_view_btn' ), 21 );

            add_action('lapa/action/shop_loop_item_action', function(){ echo '</div>'; }, 25 );
            ...
            add_action('woocommerce_single_product_summary', array( $this, 'add_sku_to_single_product' ), 8);
            ...
            add_action('woocommerce_single_product_summary', 'woocommerce_template_single_meta', 52);
            ...
            if( Lapa()->settings()->get('product_single_hide_product_title', 'no') == 'yes'){
            ...
            if( Lapa()->settings()->get('catalog_mode', 'off') == 'on'){
                // In Loop
        ...
        public function woocommerce_single_product_image_thumbnail_html( $html, $attachment_id ){
            ...
            $new_html = ' data-videolink="'.esc_attr($video_link).'"';
        ...
        public function woocommerce_loop_add_to_cart_link( $html, $product, $args ) {
            return sprintf(
        ...
        public function wrapper_start(){
            ...
            echo '<div class="row">';
            echo '<main id="site-content" class="'. esc_attr(Lapa()->layout()->get_main_content_css_class('col-xs-12 site-content')) .'">';
            echo '<div class="site-content-inner">';
            do_action( 'lapa/action/before_render_main_inner' );
        ...
        public function wrapper_end(){
            ...
            do_action( 'lapa/action/after_render_main_inner' );
            ...
            echo '</div><!-- ./container -->';
            echo '</div><!-- ./site-main -->';
            do_action( 'lapa/action/after_render_main' );
        }
        ...
        public function set_sidebar_for_shop( $sidebar ) {
            ...
            if( in_array( 'is_woocommerce', $context ) ){
                ...
                if(in_array( 'is_archive', $context ) ){
                    ...
                    $sidebar = Lapa()->settings()->get('shop_sidebar', $sidebar);

                    if(Lapa()->settings()->get('shop_global_sidebar', false)){
                        /*
                    ...
                    if(in_array( 'is_shop', $context)){
                        if( ($single_sidebar = Lapa()->settings()->get_post_meta( Lapa()->get_page_id(), 'sidebar')) && !empty($single_sidebar) ){
                            $sidebar = $single_sidebar;
                        }
                    }
                    if(in_array( 'is_product_taxonomy', $context)){
                        if( ($tax_sidebar = Lapa()->settings()->get_term_meta( get_queried_object_id(), 'sidebar')) && !empty($tax_sidebar) ){
                            $sidebar = $tax_sidebar;
                        }
                    }
                }
                ...
                elseif(in_array('is_product', $context)){
                    $sidebar = Lapa()->settings()->get('products_sidebar', $sidebar);

                    if(Lapa()->settings()->get('products_global_sidebar', false)){
                        /*
                         * Return global sidebar if option will be enable
                         * We don't need more checking in context
                         */
                    ...
                    if( ($single_sidebar = Lapa()->settings()->get_post_meta( get_the_ID(), 'sidebar')) && !empty($single_sidebar) ){
                        $sidebar = $single_sidebar;
                    }
                }
            }
        ...
        public function add_class_to_product_loop($classes, $class, $post_id){
            ...
            if ( $product ) {
                ...
                if( 'on' == Lapa()->settings()->get('woocommerce_enable_crossfade_effect') ){
                    $with_second_image = true;
                }

                $disable_second_image = lapa_get_wc_loop_prop('disable_alt_image');
                ...
                if($with_second_image && (($galleries = $product->get_gallery_image_ids()) && !empty($galleries[0]))){
                    ...
                    $loop_layout = lapa_get_wc_loop_prop('loop_layout');
                    $loop_style = lapa_get_wc_loop_prop('loop_style');
                ...
                $classes[] = 'prod-rating-' . esc_attr(Lapa()->settings()->get('woocommerce_show_rating_on_catalog', 'off'));
            }
        ...
        public function add_script_resize_image_in_loop(){
            $image_size = lapa_get_wc_loop_prop('image_size');
            if(!empty($image_size)) {
                Lapa()->images()->before_resize();
            }
        }
        ...
        public function add_second_thumbnail_to_loop(){
            ...
            $prods_masonry = lapa_get_wc_loop_prop('prods_masonry', false);
            ...
            $allow_caching = apply_filters('lapa/help/allow_product_image_cache', false);
                ...
                if($prods_masonry){
                    if($image = wp_get_attachment_image_src($product->get_image_id(), $shop_catalog_size)){
                        ...
                        $html_image_cache = sprintf(
                            '<div class="la-lazyload-image pig-m-fallback pig-m-fallback-first" data-background-image="%s"%s></div>',
                            lapa_get_relative_url($src),
                            $style
                        );
                    }
                    else{
                        $html_image_cache = '<div class="pig-m-fallback pig-m-fallback-first no-image"></div>';
                    }
                }
                else{
                    ...
                    if(!empty($ids) && isset($ids[0])){
                        if($prods_masonry){
                            if($image = wp_get_attachment_image_src($ids[0], $shop_catalog_size)){
                                ...
                                $html_image_cache .= sprintf(
                                    '<div class="la-lazyload-image pig-m-fallback pig-m-fallback-second" data-background-image="%s"%s></div>',
                                    lapa_get_relative_url($src),
                                    $style
                                );
                            }
                            ...
                            if( ($alt_image_src = wp_get_attachment_image_url($ids[0], $shop_catalog_size, false) ) && $alt_image_src ) {
                                $html_image_cache .= sprintf(
                                    '<div class="la-lazyload-image wp-alt-image" data-background-image="%s"></div>',
                                    lapa_get_relative_url($alt_image_src)
                                );
                            }
                        }
                    }
        ...
        public function add_quick_view_btn(){
            if( 'on' == Lapa()->settings()->get('woocommerce_show_quickview_btn', 'off') ){
                global $product;
                printf(
                    ...
                    esc_attr_x('Quick Shop', 'front-view', 'lapa'),
                    esc_attr_x('Quick Shop', 'front-view', 'lapa')
                );
            }
        }
        ...
        public function add_cart_btn(){
            if( Lapa()->settings()->get('catalog_mode', 'off') != 'on' && Lapa()->settings()->get('woocommerce_show_addcart_btn', 'on') == 'on' ) {
        ...
        public function add_compare_btn(){
            ...
            if( Lapa()->settings()->get('woocommerce_show_compare_btn', 'off') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Add to Compare','front-view', 'lapa'),
                    ...
                    $url = '#';
                    $text = esc_html_x('Add to Compare','front-view', 'lapa');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s">%s</a>',
                        esc_attr($css_class),
                        esc_url($url),
        ...
        public function add_wishlist_btn(){
            ...
            if(Lapa()->settings()->get('woocommerce_show_wishlist_btn', 'off') == 'on'){
                global $product;
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'lapa');
                        $class = 'add_wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'lapa');
                    ...
                    if(Lapa_WooCommerce_Wishlist::is_product_in_wishlist($product->get_id())){
                        $class = 'add_wishlist button la-core-wishlist added';
                        $url = lapa_get_wishlist_url();
                        $text = esc_html_x('View Wishlist', 'front-view', 'lapa');
                    }
                    else{
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
                add_filter('lapa/filter/product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }
        ...
        public function add_count_up_timer_to_single(){
            if(!isset($_GET['product_quickview']) && Lapa()->settings()->get('show_product_countdown')){
                global $product;
                $sale_price_dates_to = $product->get_date_on_sale_to() && ( $date = $product->get_date_on_sale_to()->getOffsetTimestamp() ) ? date( 'Y/m/d H:i:s', $date ) : '';
                if(!empty($sale_price_dates_to)){
                    echo do_shortcode('[la_countdown countdown_opts="sday,shr,smin,ssec" datetime="'. $sale_price_dates_to .'"]');
        ...
        public function check_condition_show_upsell_crosssel(){
            if ( Lapa()->settings()->get('related_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
            }
            if ( Lapa()->settings()->get('upsell_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
            }
        }

        public function add_custom_tab($tabs){
            if ( Lapa()->settings()->get('woo_enable_custom_tab', 'off') == 'on' ) {
                $tabs['custom_tab'] = array(
                    'title' => Lapa()->settings()->get('woo_custom_tab_title', esc_html_x('Custom Tab', 'front-view', 'lapa')),
                    'priority' => 40,
        ...
        public function get_custom_tab_content(){
            echo Lapa_Helper::remove_js_autop( Lapa()->settings()->get('woo_custom_tab_content', ''), true);
        }
        ...
        public function modify_availability_text_on_product_page( $availability, $product ){
            if ( ! $product->is_in_stock() ) {
                ...
                $availability = $availability = __( 'Stock: Available', 'lapa' );
            }
            return $availability;
        }
        ...
        public function add_sku_to_single_product(){
            ...
            if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ){
                ...
                <div class="product_meta-top">
                    <span class="sku_wrapper"><span class="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'lapa' ); ?></span></span>
                </div>
                <?php
            }
        }
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html_x('%s items','front-view', 'lapa') .'</span>';
        ...
        public function add_coupon_form_into_cart(){
            if ( wc_coupons_enabled() ) : ?>
                <div class="la-coupon-form">
                    <h2><?php echo esc_html_x('Coupon Code', 'front-view', 'lapa') ?></h2>
                    <p><?php echo esc_html_x('Enter your coupon code if you have one.','front-view', 'lapa')?></p>
                    <div class="la-coupon">
                        <p class="form-row form-row-wide">
                            <input type="text" class="input-text" id="coupon_code_ref" value="" placeholder="<?php echo esc_attr_x( 'Enter your coupon code..', 'front-view', 'lapa' ); ?>" />
                        </p>
                        <button type="button" class="button" id="coupon_btn_ref"><?php echo esc_html_x( 'Apply coupon', 'front-view', 'lapa' ); ?></button>
                    </div>
                </div>
            <?php endif;
        }
        ...
        public function add_more_button_to_cart_from(){
            ...
            ?>
            <input type="submit" class="button btn-clear-cart" name="clear-cart" value="<?php echo esc_attr_x('Clear Cart', 'front-view', 'lapa');?>">
            <a href="<?php echo esc_url(!empty($category_recent_link) ? $category_recent_link : wc_get_page_permalink('shop')) ?>" class="btn"><?php echo esc_html_x('Continue Shopping', 'front-view', 'lapa'); ?></a>
            <?php
        }
        ...
        private function override_page_title_bar_setting( $value, $key, $context ){
            ...
            if ( $key == 'page_title_bar_layout') {
                $new_value = Lapa()->settings()->$func($current_id, $key);
            ...
            if( Lapa()->settings()->$func($current_id, 'page_title_bar_style') == 'yes' && in_array($key, $arr2)){
                return $value;
            }

            $enable_override = Lapa()->settings()->get('woo_override_page_title_bar', 'off');
            if($enable_override == 'on'){
                $new_key = 'woo_' . $key;
                return Lapa()->settings()->get($new_key, $value);
            }
        ...
        public function woocommerce_share(){
            ...
            if(Lapa()->settings()->get('product_sharing') == 'on'){
                ...
                lapa_social_sharing($post_link,$post_title,$image);
                echo '</div>';
            }
        }
        ...
        public function woocommerce_before_shipping_calculator(){
            printf(
                ...
                esc_html_x('Calculate Shipping', 'front-view', 'lapa'),
                esc_html_x('Estimate your shipping fee *', 'front-view', 'lapa')
            );
        }
