...
class Lapa_Blog {
    ...
    public function override_setting_by_context( $value, $key, $context ){
        if($key == 'page_title_bar_layout'){
            if(is_singular('post')){
                $from_single_setting = Lapa()->settings()->get('page_title_bar_layout_post_single_global', 'off');
                $_from_current_setting = Lapa()->settings()->get_post_meta( get_queried_object_id(), $key );
